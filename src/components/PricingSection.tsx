...
const PricingSection = () => {
  ...
  const packages = [
    {
      ...
      miniPricing: [
        { label: '3 Videos', amount: '₹10,500', highlighted: true },
    ...
    },
    {
      ...
      miniPricing: [
        { label: '3 Videos', amount: '₹15,000', highlighted: true },
        { label: '5 Videos', amount: '₹24,500' },
        { label: '10 Videos', amount: '₹47,000' }
      ],
      suitable: 'Product showcases, launch films, performance creatives with premium look.',
      process: [
        'Brief & asset intake (logo, product refs)',
        'Script & storyboard (quick approval)',
        'AI generation + clean edit',
    ...
    },
    {
      ...
      miniPricing: [
        { label: '1 Video', amount: 'Starting ₹30,000', highlighted: true },
        { label: '3/5/10 Videos', amount: 'Price on request' }
      ],
      suitable: 'Luxury campaigns, hero films, high-polish cinematic brand storytelling.',
      process: [
        'Brief & asset intake (logo, product refs)',
        'Script & storyboard (quick approval)',
        'AI generation + clean edit',
  ...
  return (
    <section id="pricing" className="py-20 bg-gradient-to-b from-background via-background to-brand-dark/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {packages.map((pkg, index) => (
            <Card 
              key={pkg.id} 
              className="relative border-brand-primary/20 bg-gradient-to-b from-brand-primary/5 to-transparent hover-lift hover-glow transition-all duration-300 animate-fade-in"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <CardHeader className="text-center pb-4">
                <h3 className="text-2xl font-bold text-brand-primary mb-1">{pkg.name}</h3>
                <p className="text-sm text-brand-secondary mb-4">{pkg.subtitle}</p>
                <div className="inline-block px-3 py-1 bg-brand-primary/10 border border-brand-primary/30 rounded-full text-xs text-foreground mb-4">
                  {pkg.badge}
                </div>
                <div className="flex items-baseline justify-center gap-2">
                  <span className="text-sm text-brand-primary">Starting</span>
              ...
              <CardContent className="space-y-6">
                ...
                {/* Mini Pricing Table */}
                <div className="bg-brand-dark/50 border border-brand-primary/20 rounded-lg overflow-hidden">
                  {pkg.miniPricing.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center px-4 py-3 border-t border-brand-primary/10 first:border-t-0">
                      ...
                      <span className={`text-sm font-semibold ${item.highlighted ? 'text-brand-primary' : 'text-foreground'}`}>
                        {item.amount}
                      </span>
                    </div>
                  ))}
        ...
        {packages.map((pkg) => (
          ...
          >
            <Card className="border-brand-primary/30 bg-gradient-to-br from-brand-dark/20 via-background to-brand-primary/5 glow-card">
              <CardHeader>
                <div className="flex items-center justify-between flex-wrap gap-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button
                      ...
                      onClick={() => window.open('https://wa.me/917217832613?text=Hey%2C%20I%20want%20to%20get%20Ai%20content%20created%20for%20my%20brand.', '_blank')}
                    >
                      Consult Now
                    </Button>
                  </div>
              ...
              <CardContent>
                <div className="mb-6">
                  <p className="text-foreground font-semibold mb-2">Smart Brands Save Lakhs With AI-Powered visuals…</p>
                  <p className="text-muted-foreground">No actual shoots, crew, locations.</p>
                  <p className="text-muted-foreground">Just high-converting, viral AI Content!</p>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    <div className="bg-brand-dark/30 border border-brand-primary/20 rounded-lg p-6">
                      <h4 className="text-brand-secondary font-semibold mb-3">Suitable for</h4>
                      <p className="text-sm text-muted-foreground mb-6">{pkg.suitable}</p>

                      <h4 className="text-brand-secondary font-semibold mb-3">Process</h4>
                  ...
                  <div>
                    <div className="bg-brand-dark/30 border border-brand-primary/20 rounded-lg p-6">
                      ...
                      <div className="space-y-3">
                        {pkg.miniPricing.map((item, idx) => (
                          <div key={idx} className="flex justify-between items-center p-3 bg-brand-dark/40 border border-brand-primary/10 rounded-lg">
                            <span className="text-sm text-brand-secondary">{item.label}</span>
                      ...
                      <div className="mt-6 p-4 bg-brand-primary/5 border border-brand-primary/20 rounded-lg">
                        <h5 className="text-brand-secondary font-semibold mb-2">Notes</h5>
                        <ul className="space-y-2 text-xs text-muted-foreground">
                          <li>• The AI videos created may be showcased as testimonials on our website and social media. If you have any concerns, feel free to let us know.</li>
                          <li>• Also, any exceptional editing or product showcasing might have additional charges. If applicable, you will be notified before processing of the video.</li>
                        </ul>
                      </div>
                    </div>
                  </div>
